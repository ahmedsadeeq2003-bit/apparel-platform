...
/** Phase 8: the same alignment guarantee, for the live editor's larger
 * design-area window -- GARMENT_DESIGN_AREA_PCT/GARMENT_DESIGN_TEXTURE_OVERLAY_PCT
 * must stay internally consistent with each other exactly the way the
 * print-area pair above already is, since DesignCanvas.tsx now uses this
 * pair instead. */
describe("GARMENT_DESIGN_TEXTURE_OVERLAY_PCT", () => {
  it("is a genuinely larger window than the print-safe area", () => {
    expect(GARMENT_DESIGN_AREA_PCT.width).toBeGreaterThan(GARMENT_PRINT_AREA_PCT.width);
    expect(GARMENT_DESIGN_AREA_PCT.height).toBeGreaterThan(GARMENT_PRINT_AREA_PCT.height);
  });

  it("aligns the overlay's crop with the base photo's own design area", () => {
    const pointOnPhotoPct = { x: GARMENT_DESIGN_AREA_PCT.left, y: GARMENT_DESIGN_AREA_PCT.top };
    const pointInWindowPct = { x: 0, y: 0 };
    const pointOnOverlayImagePct = {
      x: (pointInWindowPct.x - GARMENT_DESIGN_TEXTURE_OVERLAY_PCT.left) / (GARMENT_DESIGN_TEXTURE_OVERLAY_PCT.width / 100),
      y: (pointInWindowPct.y - GARMENT_DESIGN_TEXTURE_OVERLAY_PCT.top) / (GARMENT_DESIGN_TEXTURE_OVERLAY_PCT.height / 100),
    };
    expect(pointOnOverlayImagePct.x).toBeCloseTo(pointOnPhotoPct.x, 5);
    expect(pointOnOverlayImagePct.y).toBeCloseTo(pointOnPhotoPct.y, 5);
  });

  it("aligns at the design area's bottom-right corner too", () => {
    const pointOnPhotoPct = {
      x: GARMENT_DESIGN_AREA_PCT.left + GARMENT_DESIGN_AREA_PCT.width,
      y: GARMENT_DESIGN_AREA_PCT.top + GARMENT_DESIGN_AREA_PCT.height,
    };
    const pointInWindowPct = { x: 100, y: 100 };
    const pointOnOverlayImagePct = {
      x: (pointInWindowPct.x - GARMENT_DESIGN_TEXTURE_OVERLAY_PCT.left) / (GARMENT_DESIGN_TEXTURE_OVERLAY_PCT.width / 100),
